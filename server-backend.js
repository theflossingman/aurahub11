...
const UPLOADS_DIR = path.join(__dirname, 'uploads', 'shorts');
...
const SHORTS_DATA_FILE = path.join(__dirname, 'shorts-data.json');
const USER_BIOS_FILE = path.join(__dirname, 'user-bios.json');
...
app.get('/api/export-project', (req, res) => {
    try {
        ...
        // Add data files (JSON files)
        const jsonFiles = fs.readdirSync(projectDir).filter(f => f.endsWith('.json'));
        ...
        // Add uploads/shorts directory
        if (fs.existsSync(path.join(projectDir, 'uploads'))) {
            archive.directory(path.join(projectDir, 'uploads'), 'uploads');
        }
